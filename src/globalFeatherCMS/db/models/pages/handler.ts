...
export const getPages = async () => {return await dbServerless.select().from(PagesTable)}

export const getPage = async (fullSlug: string) => {
	const cleanedSlug = cleanSlug(fullSlug);

    const results = await dbServerless.select().from(PagesTable)
        .leftJoin(PagesToSectionsM2MTable, eq(PagesToSectionsM2MTable.PageFK, PagesTable.id))
        .leftJoin(SectionInstancesTable, eq(PagesToSectionsM2MTable.SectionInstanceFK, SectionInstancesTable.id))
        .where(eq(PagesTable.slug, cleanedSlug))

    const formattedData: Page = results.reduce((agg: Page, cur) => {
